matrix puzzle maker
$Id: mulk/basic matpuzzl.m 1442 2025-06-12 Thu 10:05:28 kt $

*[man]
.caption SYNOPSIS
	matpuzzl
.caption DESCRIPTION
In this puzzle you are given a matrix of letters, up to 6 x 6, and a board with dashed and a number above each of the dashes.

The matrix represents the letter of the words you typed in, each having the same length.
The numer of words and the length are both limited to six, six words each six letters in length, but, both values don't have to be the same.

The dashes on the board form the places for each letter of each word to be written in after it has been deciphered.
The number above each dash is the sum of the coordinates of where the letter of that dash is located in the matrix.
The problem is solving the puzzle is that several coordinates have the same sum.
.caption ORIGIN
...
*entry.@
	Mulk import: "basic";
	Object addSubclass: #Cmd.matpuzzl
**Cmd.matpuzzl >> main: args
	Basic new run: #Cmd.matpuzzl.source

battleship
$Id: mulk/basic battle.m 1442 2025-06-12 Thu 10:05:28 kt $

*[man]
.caption SYNOPSIS
	battle
.caption DESCRIPTION
BATTLE is based on the popular game Battleship which is primarily played to familiarize people with the location and designation of points on coordicate plane.

BATTLE first randomly sets up the bad guy's fleet disposition on a 6 by 6 matrix or grid.
The fleet consists of six ships: Two destroyers (ships number 1 and 2) which are two units long, two cruiser (ships number 3 and 4) which are three units long and two aircraft carriers (ships number 5 and 6) which are four units long.
The program then prints out this fleet disposition in a coded or disguised format.
You then proceed to sink the various ships by typing in the coordinates (two digits, each from 1 to 6) of the place where you want to drop a bomb, if you'll excuse the expression.
The computer gices the appropriate responses (splash, hit, etc.) which you should record on a 6 by 6 matrix.
You are thus building a representation of the actural fleet disposition which you will hopefully used to decode the coded fleet disposition printed out by the computer.
Each time a ship is sunk, the computer prints out which ships have been sunk so far and also gives you a "SPLASH/HIT RATIO".
.caption ORIGIN
Basic computer games micro computer edition.
.
source from http://www.classicbasicgames.org/
.caption AUTHOR
Ray Westergard, Lawrence Hall of Science, Berkeley, California.

*entry.@
	Mulk import: "basic";
	Object addSubclass: #Cmd.battle
**Cmd.battle >> main: args
	Basic new breakFor run: #Cmd.battle.source

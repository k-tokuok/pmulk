geometric war
$Id: mulk/basic geowar.m 1442 2025-06-12 Thu 10:05:28 kt $

*[man]
.caption SYNOPSIS
	geowar
.caption DESCRIPTION
This program very loosely represent a battlefield in which you, the player, and located at point 0, 0.
There are five enemy installations that may be located anywhere from 0, 0 to 30, 30 except for a clear zone from 0,0 to 10,10.
In this game you are firing missle in a very straight path which destroys everything in its path.
If the missile flies within one unit either northwest of southeast of the target, or, of course, over the target directly, that target is destroyed.
If it is within two units of the target, the missle will be shot down and that target will relocated to a new position one unit away from its previous position in some random direction.
.caption ORIGIN
...
*entry.@
	Mulk import: "basic";
	Object addSubclass: #Cmd.geowar
**Cmd.geowar >> main: args
	Basic new run: #Cmd.geowar.source

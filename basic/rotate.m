panel rotate puzzle
$Id: mulk/basic rotate.m 1442 2025-06-12 Thu 10:05:28 kt $

*[man]
.caption SYNOPSIS
	rotate
.caption DESCRIPTION
The game of Rotate is played on a four-by-four board filled randomly with the letters A through P.

The object of the game is to put the letters in alphabetical order.
This is done by rotating groups of the four letters clockwise one position.
The group to be rotates is specified by the positional number of the letter in the upper left hand corner of the group.
You are also given one special move which permits you to exchange any two adjacent letters.
You probably don't want to use this move too early in the game; indeed, sometimes it's not necessary at all, and since you get it only one time, only you use it you can't recover.
You only move then is to type a zero to give up.

Typically, a game will tame from 20 to 30 moves to win.
I haven't figured out the worst possible case (assuming an intelligent methof of play).
.caption ORIGIN
...
*entry.@
	Mulk import: "basic";
	Object addSubclass: #Cmd.rotate
**Cmd.rotate >> main: args
	Basic new breakFor run: #Cmd.rotate.source

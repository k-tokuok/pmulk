computerized version of master mind
$Id: mulk/basic mastrmnd.m 1442 2025-06-12 Thu 10:05:28 kt $

*[man]
.caption SYNOPSIS
	mastrmnd
.caption DESCRIPTION
Master Mind is played by two people -- one is called the code-maker; the other, the code-breaker.
At the beginning of the game the code-maker forms a code, or combination of colored pegs.
He hides these from the code-breaker.
The code-breaker then attempts to deduce the code, by placing his own guesses, ont at a time, on the board.
Adter he makes a guess (by placing a combination of colored pegs on the board), the code-maker then gives the code-breaker clues to indicate how close the guess was to the code.
For every peg in the guess that's the right color and in the right position, the code-breaker gets a black peg.
For every peg in the guess that's the right color but not in the right position, the code-breaker gets a white peg.

You and the computer are in charge of code-breaker and code-maker alternately.
.caption ORIGIN
...
*entry.@
	Mulk import: "basic";
	Object addSubclass: #Cmd.mastrmnd
**Cmd.mastrmnd >> main: args
	Basic new breakFor run: #Cmd.mastrmnd.source

...
$Id: mulk/basic obstacle.m 1442 2025-06-12 Thu 10:05:28 kt $
...
A car is represented by the character '*', the obstacles are the walls (represented by excamation points and hyphens) and spaces are where the car may travel.
The car may not pass over or occupy a wall or obstacle.
The character 'S' at the upper left corner, indicates where the car starts from, and the character in the lower right corner, the 'F', is the space the car must occupy at the finish to win.
If the car tries to occupy or pass through a wall the game is lost.
When the car lands on the space occupied by the character 'F', the car has finished the course, and the game is over, and a time is caliculated.
.caption ORIGIN
...
*entry.@
	Mulk import: "basic";
	Object addSubclass: #Cmd.obstacle
**Cmd.obstacle >> main: args
	Basic new run: #Cmd.obstacle.source

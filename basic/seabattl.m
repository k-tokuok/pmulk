game of sea battle
$Id: mulk/basic seabattl.m 1442 2025-06-12 Thu 10:05:28 kt $

*[man]
.caption SYNOPSIS
	seabattl
.caption DESCRIPTION
This is the game of sea battle.
The object of the game is to destroy all of the enemy ships in your 20 by 20 area with the various weapons in your submarine's arsenal.
You must do this, however, without letting the enemy destroy yor first!

There are several interesting hazards in the game.
They include:
.item
...
The depth charges are effective to any depth, but they lose their eddectiveness over distance, so the farther you are from any ships, the better!
The sea monsters take a meandering course throuth your area that mey bring it close enough to attack you.
You rarely survive.
They also line to eat your torpedos, but missiles will kill them.

The enemy ships move on every turn, in a fixed course, unless they encounter obstacles.
They will get blown up bu by mines, and get eaten by sea monsters, too.

You have ten orders that you may give.
They are:

.caption #0: Navigation
This command allows you to move in a particular direction and distance across your area.
The direction is determined as follow:
	8 1 2
	 \|/
	7-*-3
	 /|\
	6 5 4
There are 8 directions to move in, and they are the same anytime you are asked for a course.
For example, to move north, you would use course #1.
The computer will also ask for an amount of power.
It takes 100 units of power to move your sub 1 space.
Beware of obstacles!!
If you use more than 1000 units in a turn, there is an overload danger, so be very careful!!

.caption #1: Sonar
This command has two options.
Option #1 gives directional information, showing the directions and distances in which there are enemy ships.
This is useful for shooting at long ranges, where it is difficult to tell if a ship is in direct line.

Option #0 prints out a map of your area in a square.
(It uses symbols for the map)
'*' indicates dry land.
'$' is an underwater mine. 
'\S/' is an enemy ship.
'-#-' is a sea monster.
'!H!' is your headquaters.
And finally, '(X)' is you!!!

Every so often, a '.' will appear inside the screen.
This is a sonar malfunction, and so the object there isn't identified.
If you are above 50 feet, waves will show up as '.'.

.caption #2: Torpedo control
This command allows you to shoot 1 of your 10 torpedos at enemy ships.
The computer will only require the rection to shoot, using the indicator above.
They have a range of 7-13 spaces.
One torpedo gets one ship.

.caption #3: Polaris missile control
This command allows you to launch one of your polaris missiles against the enemy.
The computer will ask for a course and fuel.
It takes 75 lbs, of fuel to boost a missle 1 space.
Since they are so much more powerful, they will completely destroy the space they land on, plus all of the immdiately adjacent ones.
Missiles destroy everything!!!

.caption #4: Manuevering
This command allows you to change the depth you're at.
You may want to do this if you are badly damages, because repairs go on twoice as quickly below 2500 ft, and above 50 ft. than in between.
You start the game at 100 ft.
You uses up about 1 power unit for every 2 ft. you change.

.caption #5: Status/damage report
This command gives you the status of your sub.
It tells you how much is left in your arsenal, which items are damages, and how much.

.caption #6: Headquarters
This command allows scuba divers from your headquaters to replenish your supply of weapons and men.
You must be at 50 ft. or less, and 2 or less spaces away to do this however, and you can only do it twice.

.caption #7: Sabotage (scuba)
This command allows you to send men out on a sabotage mission against enemy ships.
You may only go against ships within 3 spaces of you, and you must leave at least 10 men on board the sub to run it.

.caption #8: Power conversion
This command allows you to change fuel to power, or vice-versa.

.caption #9: Surrender
This command is only for cowards and traitors!!
.lineBreak
You start the game with the following supplies: 6000 units of power, 250 lbs. of rocket fuel, 10 torpedos, 3 missiles, 1 headquaters, and a random number of ships.

I left some interesting details out of the instructions, to make playing the game the first few times more interesting.

You start the game in the island's lagoon, and it is your duty to seek out and destroy the enemy at all costs!!
.caption ORIGIN
...
*entry.@
	Mulk import: "basic";
	Object addSubclass: #Cmd.seabattl
**Cmd.seabattl >> main: args
	Basic new breakFor run: #Cmd.seabattl.source

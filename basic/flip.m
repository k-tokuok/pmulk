guess the pattern
$Id: mulk/basic flip.m 1442 2025-06-12 Thu 10:05:28 kt $

*[man]
.caption SYNOPSIS
	flip
.caption DESCRIPTION
This game may be the only one so easy that even an animal could play it, yet hard for people to play even as well as random chance.
It may be useful in training the intuition, and improving gamesmanship in speculation-type activities, where each player is trying to outguess the other's behavior and stay one step ahead.

On each turn, the program first selects 'yes' or 'no', but gives you no information about its decision.
Therefore your guess on the first turn is pure chance, there is no skill involved.
But soon the program starts using patterns in your behavior, making its decisions to increase the chance of your next guess being wrong.
And to make it harder for you, the program doesn't strictly maximize its chances, but throws a little randomness into its decisions.
.caption ORIGIN
...
*entry.@
	Mulk import: "basic";
	Object addSubclass: #Cmd.flip
**Cmd.flip >> main: args
	Basic new run: #Cmd.flip.source

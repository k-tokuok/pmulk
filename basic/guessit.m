simple bluffing game versus computer
$Id: mulk/basic guessit.m 1442 2025-06-12 Thu 10:05:28 kt $

*[man]
.caption SYNOPSIS
	guessit
.caption DESCRIPTION
The object of this game is to guess an unknown number called the 'down number'.
The game is played with the numbers 1 to 11.
You will be given a hand of 5 randomly selected between 1 and 11.
The computer will have a similar hand.
The down number will always be the number not in either player hands.

You alternate moves with the computer.
On any move there are two options -- guess the down numner or ask about some number.

When a player guess the down numner the game stops.
If the guess is correct that player wins.
If the guess is no correct that player loses.

All questions about number in the other player hand must be answered truthfully.
A player may 'bruff' by asking about a number in his own hand.
The computer will sometimes do this.
...
.caption AUTHOR
Gerard Kiernan, Manhattanville College, Purchase, NY.

*entry.@
	Mulk import: "basic";
	Object addSubclass: #Cmd.guessit
**Cmd.guessit >> main: ars
	Basic new run: #Cmd.guessit.source

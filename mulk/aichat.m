...
Once activated, the chat will be performed using the input line as it is, and the following input lines can be used to perform various functions.
	...
	empty string -- enter a prompt via the wb integration
	EOF or "!" -- exit
	...
	@adjust -- change the last response
...
.caption OPTION
	...
	m NO -- Use the predefined model for number NO (default is model 0) 
	M MODEL -- Explicitly specify the model name
...
**AIChat >> setModel: opArg
	...
	self models ->:ms;
	opArg at: 'm' ->:opt, notNil? ifTrue: [ms at: opt asInteger ->model!];
	opArg at: 'M' ->opt, notNil? ifTrue: [opt ->model!];
	ms first ->model
**AIChat >> main: args
	OptionParser new init: "m:M:k:i:va" ->:op, parse: args ->args;
	self setModel: op;

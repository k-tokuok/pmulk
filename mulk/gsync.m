...
$Id: mulk gsync.m 1499 2025-12-21 Sun 21:11:23 kt $
#ja Googleドライブとの同期
...
If deletion of a file is detected, subsequent selections can be omitted by specifying the preferred side in uppercase.
.caption See Also
.summary gdrive
**#ja
.caption 書式
...
ファイルの削除が検出された場合、優先側を大文字で指定することでそれ以降の選択を省略できる。
.caption 関連項目
.summary gdrive
...
**Cmd.gsync >> copyFrom: from to: to
	...
	from pipeTo: to
**Cmd.gsync >> query
	Prompt getString: "priory to l)ocal, g)drive or s)kip" satisfy:
		[:s s empty? not and: ["lgs" includes?: (s first ->:result)]];
	result!
**Cmd.gsync >> queryIfRemoved
	defaultPriory notNil? ifTrue: [defaultPriory!];
	Prompt 
		getString: "priory to l)ocal, gdrive or s)kip, capital to set default"
...
**Cmd.gsync >> copy: name cons: cons
	...
	cons car nil? ifTrue: 
		...
		self queryIfRemoved ->ch, = 'l' 
			ifTrue: [self remove: gdriveRoot + name];
		ch = 'g' ifTrue: [self copyToLocal: name]];
	cons cdr nil? ifTrue: 
		[Out putLn: name + " gdrive removed";
		self queryIfRemoved ->ch, = 'l' ifTrue: [self copyToGdrive: name];

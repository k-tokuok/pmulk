...
$Id: mulk lang.mm 1487 2025-12-06 Sat 21:58:33 kt $
#ja Mulk言語仕様書
...
For the base class library, please refer to the relevant specification (manual topic "base").
...
なお、基盤クラスライブラリについては当該仕様書(マニュアルトピック"base")を参照の事。
...
Blocks have no inherent scope, and all method variables have scope from definition to method termination.
...
メソッド変数名は英小文字で始める。
ブロックは固有のスコープを持たず、全てのメソッド変数は定義からメソッド終端までのスコープを持つ。
...
If the radix is omitted, 10 is assumed to be specified.
...
Arguments, messages of the same priority, cascades, and assignment expressions are evaluated from left to right.
...
引数、同じ優先順位のメッセージ、カスケード、代入式は左から順に評価される。
演算の優先順位を変える場合は()を用いる。
